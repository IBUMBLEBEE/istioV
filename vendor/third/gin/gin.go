...
func (engine *Engine) LoadHTMLGlob(pattern string) {
	if IsDebugging() {
		...
		engine.HTMLRender = render.HTMLDebugWithFuncMap(engine.FuncMap)
	} else {
...
func (engine *Engine) Run(addr string) error {
	...
	server := &http.Server{Addr: addr, Handler: engine, ReadTimeout: 10 * time.Second,
		WriteTimeout: 10 * time.Second, ReadHeaderTimeout: 8 * time.Second}
	runningServers = append(runningServers, &ServerListener{
		srv: server,
		ln:  ln,
	})
...
// SIGUSR2 performs graceful reload, other signals perform graceful exit
...
func UseAdminRoute(engine *Engine) {
	engine.useAdminRoute()
}
